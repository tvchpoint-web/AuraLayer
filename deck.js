(function() {
  ...
  function setSlider(i, pct, on) {
    ...
    if (on) {
      ...
      if (label) { label.style.color = '#E8C96B'; label.style.fontSize = '24px'; label.style.transform = 'scale(1.15)'; label.style.transition = 'all 0.3s'; }
      ...
      if (label) { label.style.color = 'var(--gold)'; label.style.fontSize = '20px'; label.style.transform = 'scale(1)'; }
 ...
 var t=document.getElementById('tag-3'); if(t) t.style.opacity='1'; }, 400);
  ...
  function startAnim() {
    stopAnim();
    seqIdx = 0;
    // rAF ensures slide is painted/visible before animation begins
