(function() {
  ...
  function startAnim() {
    ...
    seqIdx  = 0;
    current = [38, 28, 20, 14];
    // Reset all sliders to starting position instantly (no transition)
    for (var i = 0; i < 4; i++) {
      var fill = document.getElementById('fill-' + i);
      var lbl  = document.getElementById('pct-'  + i);
      // Remove CSS transition temporarily so reset is instant
      if (fill) { fill.style.transition = 'none'; fill.style.width = current[i] + '%'; }
      if (lbl)  lbl.textContent = current[i] + '%';
      setActive(i, false);
    }
